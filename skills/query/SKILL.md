---
name: query
description: 'QRSPI Step 1: generate isolated research questions (queries.md) from a feature request, without reading the codebase.'
when_to_use: 'Use for the Query step of a QRSPI workflow, including refinement after Research surfaces new questions. Use only within a QRSPI workflow — a `./qrspi/<feature>/` workspace exists, or the user asks for QRSPI or names this step.'
...
3. If `queries.md` exists, read and retain its complete contents, then move it aside to `queries.md.bak`, or `queries.md.bak.<n>` with the next unused number. The agent's `Write`-only toolset can create a fresh file but can't overwrite an existing one.
4. Spawn the agent, passing the feature name and the contents of `request.md` verbatim. In refinement mode, also pass the prior queries and the New Questions verbatim. In regeneration mode, pass the prior queries verbatim and instruct the agent to preserve still-relevant questions. If the human has additional post-Spec questions, pass them verbatim as `Additional questions from the human`; do not invent or answer them. These are copied texts only — do not summarize, reword, or add research findings:
...
2. If there are Questions for the User: append them to `./qrspi/<feature>/request.md` under `## Open Questions` (create the section if needed; keep the original request intact). Then ask them directly. As each is answered, move it from `## Open Questions` to `## Clarifications` with its answer. This is a revision to intent, not to queries.md.
